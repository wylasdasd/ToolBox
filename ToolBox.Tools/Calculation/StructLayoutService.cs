...
public static class StructLayoutService
{
    ...
    public static ToolResult<StructLayoutResult> Calculate(string? structDefinition, int pack)
    {
        try
        {
            var normalizedSource = SourceParser.NormalizeSource(structDefinition ?? string.Empty);
            var kind = SourceParser.DetectLayoutKind(normalizedSource);
            var layoutKindText = kind == LayoutKind.Union ? "union" : "struct/class";
            ...
            return ToolResult<StructLayoutResult>.Ok(new StructLayoutResult(
                layoutKindText,
                result.Fields,
                result.Rows,
                result.RawSize,
        ...
        {
            return ToolResult<StructLayoutResult>.Fail(ex.Message);
        }
    }
