...
public partial class UnitConverterVM : ViewModelBase
{
    ...
    public UnitCategory CurrentCategory => GetCategory(CategoryId);
    ...
    public IEnumerable<UnitConversionRow> ConversionRows
    {
        ...
        {
            ErrorMessage = null;
            if (!TryParseInput(Input, out var inputValue))
            {
                yield break;
            }
            ...
            {
                var converted = baseValue / unit.FactorToBase;
                yield return new UnitConversionRow(unit.Label, FormatValue(converted), unit.Id == FromUnitId);
            }
        }
    }

    public string? NetworkHint => BuildNetworkHint();

    public IEnumerable<UnitDef> FileSizeUnits =>
        Categories.First(c => c.Id == "storage-si").Units.Where(u => u.Id is "b" or "kb" or "mb" or "gb" or "tb");

    public IEnumerable<UnitDef> BandwidthUnits =>
    ...
    {
        if (!TryParseInput(FileSize, out var size) || !TryParseInput(Bandwidth, out var bw) || bw <= 0)
        {
            TransferTime = string.Empty;
            return;
        }
        ...
        TransferTime = FormatDuration(seconds);
    }
    ...
    private static UnitCategory GetCategory(string id) =>
