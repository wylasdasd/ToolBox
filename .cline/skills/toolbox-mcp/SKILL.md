...
- 直接描述任务，或 `@toolbox-format`、`@toolbox-system` 等点名 Skill。
...
C# 执行、目录同步**执行**、Socket/WS、剪贴板键入、AI/OCR、SVG 页面预览——用仓库 UI，不调 MCP。

**只读 MCP**：`data_uri_image_parse`、`directory_sync_plan`。
...
工具列表应为 **33** 个；若仍 31 个，Disable MCP 后重建并 Refresh。
